...
function generateRecurringEvents() {
  ...
  // 既存のマラソンイベントを保持（手動登録分）
  const existingData = sheet.getDataRange().getValues();
  const marathonEvents = [];
  for (let i = 1; i < existingData.length; i++) {
    if (existingData[i][0] === 'MARATHON' && existingData[i][7] === true) {
      marathonEvents.push(existingData[i]);
    }
  }
  ...
  // マラソンイベントを追加（日付順に挿入）
  const allEvents = [...events, ...marathonEvents];
  ...
  Logger.log('マラソン（手動）: ' + marathonEvents.length + '件保持');
